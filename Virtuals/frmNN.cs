...
{
    public partial class frmNN : Form, IPointConnect
    {
        bool blink = false;
        ...
        public frmNN(Entity ent, ImportRealValues import, ExportRealValues export)
        {
            InitializeComponent();
            this.Width = 164;
            this.ent = ent;
        ...
        private void frmNN_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void backgroundFetch_RunWorkerCompleted(object sender, System.ComponentModel.RunWorkerCompletedEventArgs e)
        {
            ...
            if (Reals.ContainsKey("UserLevel"))
            {
                ...
                if (UserInfo.GetCurrentLevel(slevel) >= UserLevel.Оператор)
                {
                    ...
                    btnQuit.Enabled = btnChangeValue.Enabled = false;
            }
            else
                btnQuit.Enabled = btnChangeValue.Enabled = false;
            pbBar.Invalidate();
                ...
                using (frmChangeValue form = new frmChangeValue())
                {
                    ...
                    if (form.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (Math.Abs(hi - lo) > decimal.MinValue &&
                            decimal.TryParse(form.Value, out value))
                        {
